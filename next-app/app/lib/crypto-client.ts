...
export async function generateKeySubtleClient() {
  const key = await crypto.subtle.generateKey({ name: CiphertextAlgorithm.Message, length: 256 }, true, [
    'encrypt',
    'decrypt',
  ])
...
export async function getIdentityKeyPairFromHex({
  privateKeyHex,
  publicKeyHex,
}: {
  privateKeyHex: Hex
...
export async function verifyMessageAgainstPubkeyHex({
  messageBuffer,
  signature,
  publicKeyHex,
}: {
...
export async function encryptSubtleClient(
...
export async function decryptSubtleClient(
  ...
  return await crypto.subtle.decrypt({ name: CiphertextAlgorithm.Message, iv: keys.iv, tagLength: 128 }, keys.key, data)
}
