...
import {
  deleteOld,
  getImage,
  MAX_IMAGES_IN_CACHED_INDEX_DB,
  saveImage,
} from "../indexdb/media-db";
import {
  extractIdFromImageSource,
  formatFileSize,
...
export const MessageBubble = memo(
  ({
    ...
    const [displayText, setDisplayText] = useState<string>(
      message.msg.startsWith("<<<$#!") ? "" : message.msg,
    );
    const [imageSrc, setImageSrc] = useState<string | null>(null);
    const [isImagePlaceholder, setIsImagePlaceholder] =
      useState<boolean>(false);
    const [copyStatus, setCopyStatus] = useState<"idle" | "copying" | "copied">(
      "idle",
    );
    const [isLongPress, setIsLongPress] = useState<boolean>(false);
    const [menuPosition, setMenuPosition] = useState<"above" | "below">(
      "below",
    );
    ...
    useEffect(() => {
      ...
      const processMessage = async () => {
        ...
        if (mediaTypeOrNull) {
          ...
          const fileId = message.msg.slice(
            mediaCodes.START.length,
            message.msg.length - mediaCodes.END.length,
          );
          ...
          try {
            ...
            const decryptedBinary = await decryptSubtleClient(resp.data, {
              key,
              iv,
            });
    ...
    return (
      <div
        className={`flex flex-col ${isCurrentUser ? "items-end" : "items-start"} relative`}
      >
        <div
          className={`group flex ${isCurrentUser ? "justify-end" : "justify-start"} items-center gap-2 ${!isCurrentUser ? "flex-row-reverse" : ""}`}
        >
          <button
          ...
          >
            {!isCurrentUser && (
              <div className="text-xs font-medium mb-1 opacity-75 select-none flex items-center gap-1">
                <span
                  className={`${!message.isSignatureValid && "text-red-400"}`}
                >
                  {message.username}
                </span>
            ...
            {imageSrc ? (
              <div
                ...
                onClick={() =>
                  imageSrc && mediaType === "photo" && onImageClick(imageSrc)
                }
              >
                {mediaType === "photo" ? (
                  <img
        ...
        {showInfoMenu && (
          <div
            className={`absolute z-50 p-3 bg-gray-800 rounded-lg border border-gray-600 shadow-lg min-w-48 ${
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-2">
                ...
                {isMedia && imageSrc && (
                  <a
                    href={imageSrc}
                    download={`mess_and_jar-${extractIdFromImageSource(imageSrc)}${getFileExtensionFromMediaType(mediaType)}`}
